 ...
 *
 *   tag matchers           <matcher> { ";" <matcher> }   (tags_to_remove_on_tool_use)
 *              <matcher>: <category_token> [ ":" <tag_token> ]
 *
 *   category list          <category_token> { ";" <category_token> }
 *                          (allowed_tags_from_tools, accessory_tag)
 *
...
// ─── Tag matchers and category lists ──────────────────────────────────────────

/**
 * "paint_color;stat_tracker:kills" →
 *   [{ key:'paint_color', value:null }, { key:'stat_tracker', value:'kills' }]
 *
 * The `tags_to_remove_on_tool_use` grammar. Same ";" delimiter as `tags`, but a
 * bare category is legal and means "every tag in this category" — Valve's
 * worked examples use both forms, so parseTags (which rejects a pair-less
 * token) cannot serve here.
 */
function parseTagMatchers(str) {
  if (str == null || str === '') return [];
  return split(str, ';').map(entry => {
    const idx = entry.indexOf(':');
    if (idx === -1) return { key: entry, value: null };
    if (idx === 0 || idx === entry.length - 1) {
      throw new Error(`Invalid tag matcher "${entry}" in "${str}"`);
    }
    return { key: entry.slice(0, idx).trim(), value: entry.slice(idx + 1).trim() };
  });
}

/** Does a matcher select this {key,value} tag? A null value matches the whole category. */
function tagMatches(matcher, tag) {
  return matcher.key === tag.key && (matcher.value == null || matcher.value === tag.value);
}

/**
 * "paint_color;sticker" → ['paint_color', 'sticker']
 *
 * The `allowed_tags_from_tools` / `accessory_tag` grammar: bare category
 * tokens, never pairs. A ":" here is an authoring error rather than a variant
 * form — Steam matches these against a tag's *category*, so a pair would
 * quietly permit nothing at all.
 */
function parseCategoryList(str) {
  if (str == null || str === '') return [];
  return split(str, ';').map(token => {
    if (token.includes(':')) {
      throw new Error(`Invalid category token "${token}" in "${str}": expected a bare category, not a tag pair`);
    }
    return token;
  });
}
