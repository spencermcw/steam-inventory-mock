 ...
 * schema.js
 *
 * Loads Steam's wire format — dist/itemdefs.json, the transpiler's output — and
 * pre-parses every delimited string into structure.
 *
 * We deliberately ingest the wire format rather than itemdefs/*.yaml. Reading
 * the YAML would validate the transpiler's *intent*; reading its output
 * validates the bytes Steam would actually receive, which makes this loader a
 * transpiler integration test (see wiki/Tech/Provider Architecture.md).
 *
 * Loading is strict: anything Steam would reject — a bundle pointing at a
 * missing itemdefid, an unparsable exchange formula, a duplicate id — is a load
...
const DEFAULT_SCHEMA_PATH = path.resolve(__dirname, '../../dist/itemdefs.json');
...
/**
 * Structural checks that mirror what Steam would reject on upload, plus the
 * project-level invariant that no item is ever directly tradable.
 */
function validate(schema, options = {}) {
  ...
  for (const def of schema.all()) {
    ...
    if (def.tradable && !options.allowTradable) {
      errors.push(`${where}: tradable:true — no item may ever be directly tradable`);
    }
...
/**
 * @param {string|object} [source] path to a Steam itemdef JSON, or the parsed
 *   object itself (`{ appid, items: [...] }` or a bare item array).
 * @param {object} [options]
 * @param {boolean} [options.allowTradable=false] suppress the no-tradable rule
...
function loadSchema(source, options = {}) {
  ...
  if (source == null || typeof source === 'string') {
    const file = source || DEFAULT_SCHEMA_PATH;
    if (!fs.existsSync(file)) {
      throw new Error(`No itemdef schema at ${file} — run \`npm run transpile\` first`);
    }
