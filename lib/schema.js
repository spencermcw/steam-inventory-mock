...
/** Valve: itemdefids "must be lower than 1,000,000 for non-workshop items, and no bigger than 2,147,483,647". */
const MAX_ITEMDEFID = 2147483647;
const WORKSHOP_ITEMDEFID_FLOOR = 1000000;
...
/** Valve: "The default limit is 4. Larger limits are not recommended." */
const DEFAULT_ACCESSORY_LIMIT = 4;
...
class ItemDef {
  constructor(raw) {
    ...
    // Tag tools and accessories (docs/tools.html, docs/accessories.html). The
    // first two live on opposite ends of one exchange: `tags_to_remove_on_tool_use`
    // on the tool, `allowed_tags_from_tools` on the item it is applied to.
    this.tagsToRemoveOnToolUse = g.parseTagMatchers(raw.tags_to_remove_on_tool_use);
    this.allowedTagsFromTools = g.parseCategoryList(raw.allowed_tags_from_tools);
    this.accessoryTag = g.parseCategoryList(raw.accessory_tag)[0] || null;
    // Left null when the field is absent rather than defaulted here, so that an
    // explicit `accessory_limit: 0` stays distinguishable from "unspecified"
    // (the former is a schema error, the latter is Valve's default of 4). The
    // default is applied in exactly one place: accessoryLimitOrDefault.
    this.accessoryLimit = asInt(raw.accessory_limit, null);

    /** Free-form grouping token Steam shows in the client; inert here. */
    this.displayType = raw.display_type || null;
    ...
    // Restricts this promo to explicit AddPromoItem(s) calls, excluding it
    // from a GrantPromoItems sweep even when its promo rule is otherwise
    // satisfied. Independent of rule type — see Engine.grantPromoItems.
    this.grantedManually = asBool(raw.granted_manually, false);
  ...
  /** How many accessories this item may carry — Valve's default of 4 when unset. */
  get accessoryLimitOrDefault() {
    return this.accessoryLimit == null ? DEFAULT_ACCESSORY_LIMIT : this.accessoryLimit;
  }
...
function validate(schema, options = {}) {
  ...
  for (const def of schema.all()) {
    ...
    } else if (def.itemdefid > MAX_ITEMDEFID) {
      errors.push(`${where}: itemdefid exceeds Steam's maximum of ${MAX_ITEMDEFID}`);
    } else if (def.itemdefid >= WORKSHOP_ITEMDEFID_FLOOR) {
      // Valve reserves ids at or above 1,000,000 for Workshop items. A plain
      // itemdef up here will upload and then behave surprisingly, so warn
      // rather than error: we cannot tell from the wire format alone whether
      // the caller meant it.
      warnings.push(
        `${where}: itemdefid >= ${WORKSHOP_ITEMDEFID_FLOOR} is reserved for Workshop items`
      );
    }
    ...
    // A tool with nothing to write and nothing to strip still consumes itself
    // on use, so it is a live item that silently does nothing — worth saying,
    // but not worth refusing to load: Valve accepts it.
    if (
      def.type === 'tag_tool' &&
      def.tags.length === 0 &&
      def.tagGenerators.length === 0 &&
      def.tagsToRemoveOnToolUse.length === 0
    ) {
      warnings.push(`${where}: tag_tool applies no tags, rolls no tag_generators and removes none — using it does nothing`);
    }
    if (def.accessoryLimit != null && def.accessoryLimit <= 0) {
      // Not the same statement as omitting the field: an item that may hold no
      // accessories is written by leaving accessory_tag off entirely.
      errors.push(`${where}: accessory_limit must be positive (got ${JSON.stringify(def.raw.accessory_limit)})`);
    }
  }
...
module.exports = {
  loadSchema,
  MAX_ITEMDEFID,
  WORKSHOP_ITEMDEFID_FLOOR,
  validate,
  Schema,
  ItemDef,
  COMPLEX_TYPES,
  ITEM_TYPES,
  DEFAULT_ACCESSORY_LIMIT,
  asBool,
  asInt,
};
