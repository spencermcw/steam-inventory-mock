 ...
 *
 * All three are populated, from two different places: NoTrade from the item
 * definition, ItemRemoved and ItemConsumed from the row's own provenance. See
 * `flagsFor` for which comes from where, and for the one reason a row here
 * will never carry ItemRemoved.
 */
...
/**
 * SteamItemDetails_t::m_unFlags — two halves, because the two kinds of bit are
 * known in two different places.
 *
 * ItemRemoved and ItemConsumed arrive already set, on the row. They say *why
 * this row is in this result set*, which only the operation that produced it
 * can know: a row at quantity 0 could be a consumed stack, a stack spent as
 * exchange material, or the source of a split that emptied, and no amount of
 * inspecting the row separates those. The engine records the reason at the
 * point it empties, spends or destroys an instance (ITEM_FLAGS in engine.js)
 * and MockProvider serves it on the row, so this file reads the answer rather
 * than inferring one — inference being exactly what it refused to do while the
 * answer did not exist. The payoff is concrete: a tag-tool exchange under the
 * default `toolResultPolicy: 'new-instance'` returns the destroyed target and
 * its replacement as two rows of the same itemDefId, so
 * `result.find(i => i.itemDefId === target)` is a coin flip between a live item
 * and a dead id, and ItemRemoved is the only thing that tells them apart.
 *
 * NoTrade is the other half and stays computed here: it is a property of the
 * item *definition* rather than of an operation, so it comes from the itemdef's
 * `tradable` field, read back through GetItemDefinitionProperty — the same call
 * a real client would use, so this agrees with a real binding rather than
 * reaching into the schema behind it. An itemdef that does not say `tradable`
 * is not tradable, which is Valve's default and the schema's.
 *
 * Still unreported, and deliberately: Valve's ItemRemoved also covers an item
 * traded away or expired. Neither trading nor item expiry is modelled anywhere
 * in this library, so no row will ever carry the bit for those reasons —
 * ItemRemoved here means "this call removed it", never "someone else took it".
 */
function flagsFor(provider, row) {
  // Provenance, as the provider reported it. `|| 0` rather than a cast: a row
  // from a provider that does not populate m_unFlags says nothing, and nothing
  // is 0 — which is also the right answer for every row of a plain read.
  let flags = row.flags || 0;
