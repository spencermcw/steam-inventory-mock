...
class ItemInstance {
   ...
   *
   * `flags` is m_unFlags, and is a *parameter* rather than something read off
   * the instance, because the bits that belong to a result row —
   * k_ESteamItemRemoved, k_ESteamItemConsumed — say why this row is in this
   * result set, and an instance does not know that. A stack sitting at
   * quantity 0 cannot tell you whether it was consumed, spent as exchange
   * material or emptied by a split; the operation that emptied it can, and
   * passes the answer in (see ITEM_FLAGS and ResultRows in engine.js). Reads
   * that remove nothing — GetAllItems, GetItemsByID — pass nothing and report
   * 0, which for them is the fact rather than a default standing in for one.
   *
   * Carrying it here rather than in a translation layer is also what puts
   * m_unFlags on `MockProvider.getResultItems()`, which is the faithful
   * ISteamInventory mirror and the surface a napi binding maps onto:
   * SteamItemDetails_t genuinely has this field, so the handle-based client is
   * the one that would most miss it. The provider needed no change to serve it
   * — it copies engine rows through verbatim.
   *
   * One caveat on that fidelity: k_ESteamItemNoTrade is not here. It is a
   * property of the item *definition*, not of an operation, so it is read
   * through GetItemDefinitionProperty("tradable") — by the façade (flagsFor in
   * steamworks.js) and by any handle-based client that wants it.
   *
   * Deliberately absent from toJSON(): provenance describes one operation's
   * result, not the item. A save that carried "was consumed" into the next
   * session would be reporting an event that is over.
   */
  toResult(flags = 0) {
    return {
      itemId: this.itemId,
