 ...
 * provider.js
 *
 * MockProvider — Steam's async, handle-based inventory API over the local
 * engine. This is what the game client talks to until the native
 * ISteamInventory binding exists, and swapping in SteamProvider must then be a
...
const { InventoryProvider } = require('./provider-interface');
...
class MockProvider extends InventoryProvider {
   ...
   * @param {string|object} [options.schema] path to dist/itemdefs.json (default),
  ...
  constructor(options = {}) {
    super();
    const { engine, accountId = 'player', latency = 0, clock, ...engineOptions } = options;
